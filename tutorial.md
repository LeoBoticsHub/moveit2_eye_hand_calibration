# Hand-Eye Calibration with MoveIt2

The [MoveIt Calibration](http://www.github.com/moveit/moveit_calibration) package provides plugins and a graphical
...
![Hand-Eye Calibration Demo](images/hand_eye_calibration_demo.jpg)
...
If you haven't already done so, be sure to complete the steps in docs.
Also, set your arm up to work with MoveIt2.
...
```bash
git clone git@github.com:LeoBoticsHub/moveit2_hand_eye_calibration.git
```
...
### 1. Launch RViz and Load Calibration Plugin

Launch the appropriate MoveIt demo for your robot.
In the RViz "Panels" menu, choose "Add New Panel":

![Add New Panel](images/choose_new_panel.png)

Then, select the "HandEyeCalibration" panel type:

![Add HandEyeCalibration Panel](images/add_handeye_panel.png)

The panel will be added with the "Target" tab active.

### 2. Create and Print a Target
...
![ArUco Target Example](images/aruco_target_handeye_panel.png)
...
display, and set it to listen to the `/moveit_visual_tools` topic. (It may not appear immediately.)
...
The __Calibrate__ tab provides the tools to collect the dataset and calculate and export the calibration. At this point,
...
![Calibrate Tab](images/calibrate_tab.png)

On the __Calibrate__ tab, you can select which calibration solver to use in the "_AX=XB Solver_" drop-down. The Daniilidis
solver (from the paper referenced, above) is the default and is a good choice in most situations. The "Planning Group"
is the joint group that will be recorded, so should be set to the appropriate group for the arm (for example, in the
...
Next, you can move the arm to a new pose using the __MotionPlanning__ panel, or use your robot's teaching pendant or free
...
calibration (see the _Daniilidis_ paper, linked above, for the explanation why).
